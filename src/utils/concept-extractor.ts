...
export class ConceptExtractor {
  ...
  extractConcepts(
    ...
    steps.push(
      `Computed TF-IDF scores for ${Object.keys(tfidfScores).length} terms`,
    );
    ...
    if (conceptCandidates.size < 3) {
      fallbackUsed = true;
      warnings.push(
        "Insufficient concepts from NLP analysis, using keyword fallback",
      );
    ...
    steps.push(
      `Filtered from ${conceptsBeforeFiltering} to ${concepts.length} concepts`,
    );
