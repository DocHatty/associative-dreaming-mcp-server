    ...
    formatScaffoldOutput(toolName, result) {
        ...
        switch (toolName) {
            case "semantic_drift":
                ...
                metadata.push(`Target distance: ${((result.driftDistance || 0) * 100).toFixed(0)}%`);
                if (result.computedCandidates && result.computedCandidates.length > 0) {
                    metadata.push(`Embedding provider: ${result.embeddingProvider || "unknown"}`);
                    metadata.push(`Computed candidates at target distance:`);
                    result.computedCandidates.forEach((c, i) => {
                        metadata.push(`  ${i + 1}. "${c.concept}" (${(c.distance * 100).toFixed(0)}% distant)`);
                    });
                }
                else {
                    metadata.push(`Computed candidates: None (using hints only)`);
                }
                metadata.push(`Hints provided: ${result.associationHints?.length || 0}`);
                metadata.push(`Bridges suggested: ${result.bridgeSuggestions?.length || 0}`);
                break;
            case "bisociative_synthesis":
                ...
                metadata.push(`Pattern hint: ${result.suggestedPattern || "auto-select"}`);
                break;
            ...
            case "serendipity_scan":
                ...
                metadata.push(`Novelty target: ${((result.noveltyThreshold || 0.5) * 100).toFixed(0)}%`);
                metadata.push(`Concepts extracted: ${result.extractedConcepts?.length || 0}`);
                break;
            case "meta_association":
                metadata.push(`Tool: meta_association`);
                metadata.push(`Prior outputs: ${result.priorOutputs?.length || 0}`);
                metadata.push(`Chaos intensity: ${((result.chaosIntensity || 0.7) * 100).toFixed(0)}%`);
                break;
        }
