...
import { generateBisociativeBridgeScaffold, formatScaffoldAsPrompt, } from "../prompts/creative-scaffolds.js";
...
export class BisociativeSynthesisTool {
    ...
    performSynthesis(input) {
        ...
        const patternInfo = this.identifyPattern(matrixA, selectedMatrixB, blendType);
        // Generate the LLM scaffold - this is the key change
        const scaffold = generateBisociativeBridgeScaffold(matrixA, selectedMatrixB, matrixA, // Use matrixA as the user context for now
        patternInfo.name);
     ...
     */
    identifyPattern(matrixA, matrixB, preferredPattern) {
        ...
        const scoredPatterns = ISOMORPHIC_PATTERNS.map((pattern) => {
            ...
            for (const hint of pattern.hints) {
                if (normalizedA.includes(hint))
                    score += 2;
                if (normalizedB.includes(hint))
                    score += 2;
            }
