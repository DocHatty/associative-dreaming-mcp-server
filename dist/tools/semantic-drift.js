...
export class SemanticDriftTool {
    ...
    async performDrift(input) {
        ...
        if (learnedParams &&
            ...
            transparency.addComputation(`Applied learned parameters from ${learnedParams.sampleSize} previous feedback samples (avg quality: ${learnedParams.avgQuality?.toFixed(1) || "N/A"}/10)`, "parameter-learning", 0.95, 0);
            ...
            if (learnedParams.optimalMagnitude !== undefined) {
                ...
                transparency.addComputation(`Drift magnitude adjusted to learned optimal: ${(driftMagnitude * 100).toFixed(0)}%`, "parameter-adjustment", 0.9, 0);
            }
            if (learnedParams.optimalTemperature !== undefined) {
                temperature = learnedParams.optimalTemperature;
                transparency.addComputation(`Temperature adjusted to learned optimal: ${(temperature * 100).toFixed(0)}%`, "parameter-adjustment", 0.9, 0);
            }
        }
        // 
        ...
        transparency.addComputation(`Extracted ${extraction.concepts.length} concepts from anchor using ${extraction.extractionMethod}`, extraction.extractionMethod, extraction.confidence, extractionTime);
        ...
        const associationHints = await hintService.getAssociationHints(anchorConcept, driftMagnitude, 5);
        const bridgeObjects = driftMagnitude > 0.6
            ? hintService.getCrossDomainBridges(anchorConcept, 5)
            : [];
        const bridgeSuggestions = bridgeObjects.map((b) => `${b.from} ↔ ${b.to}: ${b.bridge}`);
        const hintsTime = Date.now() - startHints;
        transparency.addComputation(`Gathered ${associationHints.length} graph-based hints (${associationHints.filter((h) => h.source === "graph").length} from graph structure, ${associationHints.filter((h) => h.source === "embedding").length} from embeddings) and ${bridgeSuggestions.length} cross-domain bridges`, "hint-matching", 0.9, hintsTime);
        // ═══════════════════════════════════════════════════════════════════
        ...
        try {
            ...
            if (candidates.length > 0) {
                computedCandidates = await embeddingService.findConceptsAtDistance(anchorConcept, driftMagnitude, candidates.slice(0, 100), // Limit to 100 most recent for performance
                0.15);
                const embeddingTime = Date.now() - startEmbedding;
                transparency.addComputation(`Computed semantic distances for ${candidates.length} concepts using ${embeddingProvider}`, "embedding-distance", computedCandidates.length > 0 ? 0.95 : 0.5, embeddingTime);
                if (computedCandidates.length > 0) {
                    transparency.addComputation(`Found ${computedCandidates.length} concepts at target distance ${(driftMagnitude * 100).toFixed(0)}%`, "distance-filtering", 0.9, 0);
                }
            }
            else {
                transparency.addComputation("No graph candidates yet - using hints only", "embedding-distance", 0.3, Date.now() - startEmbedding);
            }
        }
        catch (error) {
            // Embedding service failed - fall back to hints only
            transparency.addWarning(`Embedding service error: ${error instanceof Error ? error.message : "Unknown error"}`);
            transparency.addComputation("Falling back to hint-based suggestions", "embedding-fallback", 0.4, Date.now() - startEmbedding);
        }
        // 
        const scaffold = generateSemanticLeapScaffold(anchorConcept, driftMagnitude, anchorConcept, temperature);
        const llmPrompt = formatScaffoldAsPrompt(scaffold);
        transparency.addLLMDependency("Generate creative semantic leap", "Creative reasoning requires LLM to explore conceptual space and make justified leaps", "required", 2000);
        // 
        const { suggestedDirection, explorationPath } = this.generateDriftGuidance(anchorConcept, driftMagnitude, associationHints, bridgeSuggestions);
        // 
        const explanation = this.createExplanation(anchorConcept, driftMagnitude, temperature, associationHints, bridgeSuggestions, extraction);
        // 
        this.updateDreamGraph(anchorConcept, suggestedDirection, explorationPath, driftMagnitude);
        // 
        ...
        const transparencyReport = transparency.build(overallConfidence, `${reasoning}. Drift guidance computed, creative leap requires LLM.`);
        return {
            scaffold,
            llmPrompt,
            anchorConcept,
            ...
            computedCandidates: computedCandidates.length > 0
    ...
    generateDriftGuidance(anchor, driftMagnitude, hints, bridges) {
        const path = [anchor];
    ...
    createExplanation(anchor, driftMagnitude, temperature, hints, bridges, extraction) {
        const distanceDesc = driftMagnitude < 0.4
...
- How this reframes the original anchor`;
    ...
    updateDreamGraph(anchor, suggestedDirection, explorationPath, driftMagnitude) {
