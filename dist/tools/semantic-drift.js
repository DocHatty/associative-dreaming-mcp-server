...
export class SemanticDriftTool {
    ...
    performDrift(input) {
        ...
        for (let hop = 0; hop < numberOfHops; hop++) {
            ...
            if (nextHop.concept === currentConcept ||
                ...
                explanationSteps.push(`      Distance: ${(nextHop.distance * 100).toFixed(0)}% | ${cleanReason}`);
            }
            driftPath.push(currentConcept);
            visitedConcepts.add(currentConcept.toLowerCase().trim());
        }
     ...
     */
    findNextConcept(currentConcept, driftMagnitude, temperature, hopNumber, totalHops, visitedConcepts) {
        ...
        let unvisitedAssociations = associations.filter((a) => !visitedConcepts.has(a.concept.toLowerCase().trim()));
        ...
        if (driftMagnitude >= 0.7) {
            const highDistanceCandidates = unvisitedAssociations.filter((a) => a.distance > 0.6);
        ...
        if (candidates.length === 0) {
            candidates = unvisitedAssociations.filter((a) => a.distance >= targetMinDistance - 0.2);
        }
     ...
     */
    getWeightedAssociations(concept) {
     ...
     * V2.1: Accepts visitedConcepts Set for loop detection
     */
    selectCrossDomainBridge(currentConcept, driftMagnitude, visitedConcepts) {
        const normalized = currentConcept.toLowerCase();
        // Find bridges that start from current concept and lead to unvisited concepts
        const relevantBridges = CROSS_DOMAIN_BRIDGES.filter((b) => b.from.toLowerCase() === normalized &&
            b.distance >= driftMagnitude - 0.2 &&
            !visitedConcepts.has(b.to.toLowerCase()));
        if (relevantBridges.length > 0) {
            const bridge = relevantBridges[Math.floor(Math.random() * relevantBridges.length)];
            return {
                concept: bridge.to,
        ...
        const reverseBridges = CROSS_DOMAIN_BRIDGES.filter((b) => b.to.toLowerCase() === normalized &&
            b.distance >= driftMagnitude - 0.2 &&
            !visitedConcepts.has(b.from.toLowerCase()));
        if (reverseBridges.length > 0) {
            const bridge = reverseBridges[Math.floor(Math.random() * reverseBridges.length)];
            return {
                concept: bridge.from,
                distance: bridge.distance,
                reason: bridge.reason,
        ...
        if (driftMagnitude > 0.6 && CROSS_DOMAIN_BRIDGES.length > 0) {
            const highDistBridges = CROSS_DOMAIN_BRIDGES.filter((b) => b.distance >= 0.7);
            if (highDistBridges.length > 0) {
                const bridge = highDistBridges[Math.floor(Math.random() * highDistBridges.length)];
     ...
     * V2.1: Accepts visitedConcepts Set for loop detection
     */
    jumpToDomain(currentConcept, driftMagnitude, visitedConcepts) {
        // Pick a random concept from the dictionary that hasn't been visited
        const allConcepts = Object.keys(WEIGHTED_ASSOCIATIONS);
        const unvisitedConcepts = allConcepts.filter((c) => !visitedConcepts.has(c.toLowerCase()));
        // If all concepts visited (very unlikely), just pick any
        const conceptPool = unvisitedConcepts.length > 0 ? unvisitedConcepts : allConcepts;
     ...
     */
    updateDreamGraph(anchorConcept, newConcept, driftPath, avgDistance) {
